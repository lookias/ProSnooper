...
begin
  ...
  try
   http.Get('http://'+frmLogin.cbServer.Text+'/wormageddonweb/Game.asp?Cmd=Create&Name='+ HostedGameName +'&HostIP='+IP+'&Nick='+frmLogin.cbUser.Text+'&Chan='+ StringReplace(frmlogin.cbchan.Text,'#','',[]) +'&Loc='+ IntToStr(frmLogin.cbflag.ItemIndex) +'&Type=0&Pass=0');
   SL := TStringList.Create;
   ...
   for I := 1 to Sl.Count-2 do begin
     W.Text := Sl[I];
     if W[3] = IP then
      if W[2] = frmLogin.cbUser.Text then
       GameID := W[7];
   end;
