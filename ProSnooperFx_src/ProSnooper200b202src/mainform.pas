...
begin
  AddRichLine(rechat,MakeTimeStamp+'ProSnooperFx error: '+e.Message+' ['+Sender.ClassName+']');
end;
...
begin
   // ProSnooperFx hack: don't move the scrollbar
...
ans := MessageDlg('Do you really want to quit?', mtConfirmation,[mbYes, mbNo], 0);
 if ans = mrNo then
  ...
  DockHandler.SaveDesktop('\Software\ProSnooperFx');
  
  if frmMain.WindowState <> wsMaximized then begin
   frmLogin.SetRegistryData(HKEY_CURRENT_USER,'\Software\ProSnooperFx','Pos1',rdInteger,Height);
   frmLogin.SetRegistryData(HKEY_CURRENT_USER,'\Software\ProSnooperFx','Pos2',rdInteger,Top);
   frmLogin.SetRegistryData(HKEY_CURRENT_USER,'\Software\ProSnooperFx','Pos3',rdInteger,Left);
   frmLogin.SetRegistryData(HKEY_CURRENT_USER,'\Software\ProSnooperFx','Pos4',rdInteger,Width);
   frmLogin.SetRegistryData(HKEY_CURRENT_USER,'\Software\ProSnooperFx','Buddies',rdString,frmSettings.lbBuddies.Items.CommaText);
  end;

  if frmMain.WindowState = wsMaximized then
   frmLogin.SetRegistryData(HKEY_CURRENT_USER,'\Software\ProSnooperFx','WindowState',rdString,'Maximized')
  else if frmMain.WindowState = wsMinimized then
   frmLogin.SetRegistryData(HKEY_CURRENT_USER,'\Software\ProSnooperFx','WindowState',rdString,'Minimized')
  else
   frmLogin.SetRegistryData(HKEY_CURRENT_USER,'\Software\ProSnooperFx','WindowState',rdString,'Normal');

  irc.Quit('ProSnooperFx');
...
begin
 if lbIgNore.Items.IndexOf(NickName) = -1 then begin
  ...
  if Pos(Lowercase(frmLogin.cbUser.Text),Lowercase(Content)) <> 0 then begin  // if highlighted
   ...
   CoolTrayIcon1.ShowBalloonHint('ProSnooperFx',Nickname+' highlighted you.',bitInfo,10);
  end; //end highlight
...
begin
   if lbIgnore.Items.IndexOf(NickName) = -1 then begin
     ...
     CoolTrayIcon1.ShowBalloonHint('ProSnooperFx','You received a private message.',bitInfo,10);
     if frmSettings.cbBlink.Checked then
     CoolTrayIcon1.CycleIcons := True;
...
begin
  if frmSettings.lbBuddies.Items.IndexOf(NickName) <> -1 then begin
    ...
    CoolTrayIcon1.ShowBalloonHint('ProSnooperFx',Nickname+' logged on.',bitInfo,10);
  end;
...
begin
 CoolTrayIcon1.ShowBalloonHint('ProSnooperFx','ProSnooperFx has been minimized to the tray.',bitInfo,10);
 frmMain.Hide;
end;
...
begin
  frmLogin.SetRegistryData(HKEY_CURRENT_USER,'\Software\ProSnooperFx','AutoLogin',rdString,BoolToStr(AutoLogin1.Checked))
end;
...
begin
 ...
 DockHandler.LoadDesktop('\Software\ProSnooperFx');
